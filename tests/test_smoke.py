...
# ──────────────────────────────────────────────────────────────────────────────
# v0.3.1 follow-up — ExtractFloorError auto-retry in pipeline
# ──────────────────────────────────────────────────────────────────────────────
def test_extract_floor_error_caught_in_pipeline():
    """Pipeline must reference extract.ExtractFloorError in a try/except
    block on stage 3, so the CHANGELOG promise of "triggers extract retry"
    actually holds."""
    from pathlib import Path
    p = Path(__file__).resolve().parents[1] / "src/keynote_recap/pipeline.py"
    text = p.read_text()
    assert "extract.ExtractFloorError" in text, (
        "pipeline.py must catch extract.ExtractFloorError; otherwise stage 3's "
        "own hard-floor abort is uncatchable and the run dies on first floor breach."
    )


def test_extract_floor_error_retry_uses_directive():
    """The catch block must call extract.run(... retry_context=...) to
    inject the [RETRY GUIDANCE] block — same pattern as the stage-5.5
    Tier-1 retry."""
    from pathlib import Path
    p = Path(__file__).resolve().parents[1] / "src/keynote_recap/pipeline.py"
    text = p.read_text()
    # the catch block we just added must wire retry_context through
    assert "extract.run(state, config, retry_context=" in text


def test_extract_floor_error_respects_retry_count_idempotency():
    """The new catch block must guard on state.extract_retry_count == 0,
    matching the existing Tier-1 retry's idempotency contract — otherwise
    a flaky model could loop infinitely."""
    from pathlib import Path
    p = Path(__file__).resolve().parents[1] / "src/keynote_recap/pipeline.py"
    text = p.read_text()
    # find the ExtractFloorError catch block
    idx = text.find("except extract.ExtractFloorError")
    assert idx >= 0
    # within the next ~2000 chars (the catch body), the retry count guard
    # must appear before extract.run is called
    block = text[idx: idx + 2500]
    guard_idx = block.find("state.extract_retry_count == 0")
    retry_idx = block.find("extract.run(state, config, retry_context=")
    assert guard_idx >= 0 and retry_idx >= 0 and guard_idx < retry_idx, (
        "ExtractFloorError catch must guard on extract_retry_count == 0 BEFORE retrying"
    )

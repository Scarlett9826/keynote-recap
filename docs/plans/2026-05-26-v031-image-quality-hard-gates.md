...
def test_v031_section_fit_subsection_grain():
    ...
    场景：## 八、ACME空调 / ### 8.3 制造底气 — 武汉家电智能工厂
    frame caption: 'ACME家电智能工厂介绍片段...'
...
![（插播官方渲染）ACME家电智能工厂介绍片段：磁悬浮主板装配线](frames/factory.jpg)
"""
...
5.5.4 检查从 ## 章节级降到 ### 子节级，消除 §8 ACME空调 / §8.3 智能工厂
此类合理跨子主题的误报。
"
```
...
❌ 错误（v0.3.0 漂移）：
![ACMEYU7 GT品牌代言人舒淇宣传片：画面显示「舒淇 ACME汽车品牌代言人」字幕，舒淇坐在红色沙发上阅读，室内场景，正式确认其ACME汽车品牌代言人身份。](frames/frame_00902.jpg)
```
...
rm -rf output/xiaomi_2026_05_20_v031
cp -r output/acme-launch-2026 output/xiaomi_2026_05_20_v031
# 改 state.json 让它从 stage 3 开始
.venv/bin/python -c "
import json, pathlib
p = pathlib.Path('output/xiaomi_2026_05_20_v031/state.json')
...
OPENAI_API_KEY=$KEY .venv/bin/keynote-recap recap-and-verify \
  ...
  --output-dir ./output/xiaomi_2026_05_20_v031 \
